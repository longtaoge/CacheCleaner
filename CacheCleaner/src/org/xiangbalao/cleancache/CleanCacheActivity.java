...
public class CleanCacheActivity extends SwipeBackActivity {
		...
		public void onGetStatsCompleted(PackageStats pStats, boolean succeeded)
			...
			if (cache>=0) {
				try {
					...
					Log.i("log1", packInfo.applicationInfo.loadLabel(pm)
							.toString());
					msg.obj = temp_cache;
	...
	private class CacheAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			holder.tv_code.setText("应用大小：" + String.valueOf(info.codesize));
			holder.tv_data.setText("数据大小：" + String.valueOf(info.datasise));

			holder.tv_cache.setText("缓存大小：" + String.valueOf(info.cachesize));
			return view;
		}
